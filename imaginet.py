...
def load_dataset(path, size=(32,32)):
    X = load_phyre_rollouts(path, flat=False)
...
#%%
dataloader = load_dataset("phyre_all_obs", size=(32,32))
#%%
model = ImagiNet()
#%%
...
for e in range(100):
    ...
    for i, (X, Y) in enumerate(dataloader):
        ...
        if not i%20:
            ...
            plt.imshow(T.cat((X[0,0], T.ones(32,1), Y[0].detach(), T.ones(32,1), Z[0].detach()), dim=1))
            plt.show()
# %%
T.save(model.state_dict(), "saves/imaginet-c16-all.pt")
